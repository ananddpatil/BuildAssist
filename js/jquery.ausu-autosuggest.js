...
(function($){
    $.fn.autosugguest = function(config) {
	   ...
	   methodType: 'POST',
        ...
        $('<div class="ausu-suggestionsBox"><img src="'+SITEROOT+'/templates/default/images/arrowUp.png" /><ul></ul></div>').appendTo('.' + config.className);
        ...
        $(".ausu-suggestionsBox > ul li").live("click", function()
        {
			...
			//var word=rval.split("##**##")
			//alert(word[0]);
            fieldParent.children('input:eq(0)').val(stringval);
            if (config.rtnIDs==true) 
			{
				var Field=fieldParent.children('input:eq(1)').attr('id');
				var value= $(this).attr("id");
				//alert(Field);
				if( Field == "AI_MobileId")
				{
					jQuery.get(SITEROOT+"/modules/Booking/return_data.php",{Field:Field,value:value},function(data){jQuery("#AID").html(data);});
				}
				if( Field == "FIINFO_Financer_BankId")
				{
					jQuery.get(SITEROOT+"/modules/Booking/return_data.php",{Field:Field,value:value},function(data){jQuery("#FID").html(data);});
				}
